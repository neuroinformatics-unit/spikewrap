...
class TestWhiten(BaseTest):
    ...
    def test_whitening_consistency(self, session):
        ...
        W_sw = np.array(spikewrap_whitened._preprocessed["grouped"]["1-raw-whiten"]._kwargs["W"])  # Whitening matrix
        ...
        si_whitened = whiten(raw_recording, **whitening_kwargs)  # Apply SpikeInterface whitening
        ...
        assert (si_data.shape == sw_data.shape), "Shape mismatch between SpikeInterface and Spikewrap outputs"
        assert np.allclose(si_data, sw_data, atol=1e-5), "Data mismatch: whitened traces differ"
        assert np.allclose(W_si, W_sw, atol=1e-5), f"Whitening matrix mismatch: difference norm {np.linalg.norm(W_si - W_sw):.4f}"
